...
def scrape_fx_frankfurter():
    ...
    try:
        ...
        if r2:
            ...
            for pair,(ccy,direct) in {"EUR/USD":("EUR",False),"GBP/USD":("GBP",False),
                "USD/JPY":("JPY",True),"USD/CAD":("CAD",True),"AUD/USD":("AUD",False),
                "NZD/USD":("NZD",False),"USD/CHF":("CHF",True)}.items():
...
def scrape_fred_all():
    ...
    wti = fred("DCOILWTICO")
    dxy = fred("DTWEXBGS")
    # Gold via Yahoo Finance (FRED series unreliable)
    gold = None
    try:
        _gr = SESSION.get("https://query1.finance.yahoo.com/v8/finance/chart/GC=F",
                          params={"interval":"1d","range":"1d"}, timeout=10)
        if _gr.status_code == 200:
            gold = round(_gr.json()["chart"]["result"][0]["meta"]["regularMarketPrice"], 2)
    except: pass
    for name,val in [("VIX",vix),("WTI",wti),("Gold",gold),("DXY",dxy)]:
        if val: log.info(f"  {name} = {val}")
    global_ind = {k:v for k,v in {"vix":vix,"wti":wti,"gold":gold,"dxy":dxy,"us10y":y10}.items() if v}
    log.info(f"[FRED] macro:{sum(len(v) for v in macro.values())} | yields:{len(yields)} ccys")
    return macro, yields, global_ind
...
def _parse_rss(url, source_name, max_items=10):
    ...
    try:
        ...
        for item in root.findall(".//item")[:max_items]:
            ...
            desc  = _re.sub(r"<[^>]+>","",( item.findtext("description","") or "").strip())[:400]
...
def _classify(title, body):
    ...
    ccys = []
    if any(k in text for k in ["fed","powell","dollar","usd","trump","fomc"]): ccys.append("USD")
    if any(k in text for k in ["ecb","lagarde","euro","eurozone"]):             ccys.append("EUR")
    if any(k in text for k in ["boe","bailey","sterling","pound"]):             ccys.append("GBP")
    if any(k in text for k in ["boj","ueda","yen","japan"]):                    ccys.append("JPY")
    if any(k in text for k in ["rba","bullock","australia","aussie"]):          ccys.append("AUD")
    if any(k in text for k in ["rbnz","new zealand","kiwi"]):                   ccys.append("NZD")
    if any(k in text for k in ["boc","canada","loonie","crude","oil"]):         ccys.append("CAD")
    if any(k in text for k in ["snb","swiss","franc","chf"]):                   ccys.append("CHF")
    if not ccys: ccys = ["USD"]
    direction = ("positive" if any(k in text for k in ["hike","hawkish","beat","surge","peace","deal","ceasefire"])
                 else "negative" if any(k in text for k in ["cut","dovish","miss","collapse","war","recession","sanction"])
                 else "mixed")
    cat = ("Central Bank" if any(k in text for k in ["fed","ecb","boj","boe","rba","rbnz","boc","snb","interest rate","rate"])
           else "Geopolitics" if any(k in text for k in ["iran","israel","trump","war","sanction","middle east","ukraine","tariff","hormuz"])
           else "Macro")
...
def save_news_cache(news_list):
    ...
    try:
        with open(DATA_FILE.parent/"news_cache.json","w",encoding="utf-8") as f:
...
def save_global_indicators(ind, fx):
    data = {"vix":ind.get("vix"),"wti":ind.get("wti"),"gold":ind.get("gold"),
            "dxy":ind.get("dxy"),"us10y":ind.get("us10y"),
...
def save_risk_sentiment(vix_val):
    if vix_val and vix_val > 30: label,score = "RISK-OFF",-3
    elif vix_val and vix_val > 25: label,score = "MILD RISK-OFF",-2
    elif vix_val and vix_val > 20: label,score = "MILD RISK-OFF",-1
    elif vix_val and vix_val > 17: label,score = "NEUTRAL",0
    elif vix_val and vix_val > 12: label,score = "MILD RISK-ON",1
    elif vix_val: label,score = "RISK-ON",2
    else: label,score = "NEUTRAL",0
    data = {"vix":vix_val,"score":score,"label":label,
            "factors":[{"name":"VIX Volatility Index","score":score,"desc":f"VIX {vix_val} — {label}"}],
            "updated":datetime.now(timezone.utc).strftime("%d %b %Y %H:%M UTC")}
    try:
        with open(DATA_FILE.parent/"risk_sentiment.json","w",encoding="utf-8") as f:
            json.dump(data,f,indent=2)
        log.info(f"✓ risk_sentiment.json — {label} (VIX {vix_val})")
...
def run_full_scrape(currencies=None):
    ...
    _f = scrape_fred_all()
    fred_macro,fred_yields,global_ind = _f[0],_f[1],(_f[2] if len(_f)>2 else {})
    ...
    log.info(f"\n── Risk Sentiment ──")
    save_risk_sentiment(global_ind.get("vix"))
