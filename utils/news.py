...
# On renomme le fichier pour éviter d'écraser le news_cache.json central de la racine
CACHE_FILE = "newsapi_raw_cache.json"
    ...
    if os.path.exists(CACHE_FILE):
        try:
            with open(CACHE_FILE, "r", encoding="utf-8") as f:
                ...
                # Utilisation de timezone.utc pour éviter les avertissements de datetime naïf
...
def fetch_news(query="forex OR fx OR currency", limit=6):
    ...
    # Fallback sur le cache si l'API échoue ou si NEWS_API_KEY n'est pas configurée
    return cached
