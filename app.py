...
with tab_compare:
    ...
    if base_ccy == quote_ccy:
        ...
        expected  = (spreads[2]*regime_tuple[1])+(spreads[1]*regime_tuple[2])+(spreads[0]*regime_tuple[3])+wti_bonus
        ...
        st.markdown(f"""
        <div style="background:linear-gradient(180deg,var(--panel),var(--panel-2));padding:16px;
          ...
          <div style="height:10px;background:rgba(255,255,255,0.03);border-radius:999px;margin-top:10px;overflow:hidden;">
...
with tab_insights:
    ...
    for col, items, color, arrow in zip(
        st.columns(2),
        [top_bull, top_bear],
        ["#10b981","#f43f5e"],
        ["▲","▼"]
        ...
        with col:
            if not items:
                st.info("Aucune paire détectée.")
            for item in items:
                pct = item.get("score_pct",0)
                raw = item.get("raw",0)
                st.markdown(f"""
                <div style="padding:12px;border-radius:10px;background:linear-gradient(180deg,var(--panel),var(--panel-2));
                            margin-bottom:8px;border:1px solid rgba(255,255,255,0.05);">
                  <div style="display:flex;justify-content:space-between;align-items:center;">
                    <div>
                      <div style="font-weight:800;font-family:'JetBrains Mono',monospace;color:#fff;">{item['pair']}</div>
                      <div style="color:#94a3b8;font-size:12px;margin-top:2px;">Score brut: {raw:.2f}</div>
                    </div>
                    <div style="color:{color};font-weight:800;font-size:16px;">{arrow} {pct}%</div>
                  </div>
                  <div style="height:4px;background:rgba(255,255,255,0.05);border-radius:999px;margin-top:10px;overflow:hidden;">
                    <div style="width:{pct}%;height:100%;background:{color};border-radius:999px;opacity:0.7;"></div>
                  </div>
                </div>
                """, unsafe_allow_html=True)
